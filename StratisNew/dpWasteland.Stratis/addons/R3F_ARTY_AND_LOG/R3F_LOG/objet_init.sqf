...
if ({_objet isKindOf _x} count R3F_LOG_CFG_objets_deplacables > 0) then
{
	...
	_objet addAction [("<t color=""#dd0000"">" + STR_R3F_LOG_action_delete_objet + "</t>"), "addons\R3F_ARTY_AND_LOG\R3F_LOG\objet_deplacable\delete.sqf", nil, 5, false, true, "", "R3F_LOG_objet_addAction == _target && R3F_LOG_action_deplacer_objet_valide && !(_target getVariable ['objectLocked', false])"];
