...
aActionsIDs = aActionsIDs + [player addAction["<t color='#FFFF00'>Activate Spawn</t>","client\functions\bases\fn_baseAction.sqf",["ACTIVATE"],1.5,true,true,"",'_nearestFlag = nearestObject [player, "FlagPole_F"];  _baseSide = _nearestFlag getVariable ["BASE_SIDE", civilian]; _baseUsers = _nearestFlag getVariable ["BASE_USERS", []]; !(isNull _nearestFlag) && (_nearestFlag distance player < 5) && _baseSide == playerSide && !((getPlayerUID player) in _baseUsers)']];
aActionsIDs = aActionsIDs + [player addAction["<t color='#FFFF00'>Deactivate Spawn</t>","client\functions\bases\fn_baseAction.sqf",["DEACTIVATE"],1.5,true,true,"",'_nearestFlag = nearestObject [player, "FlagPole_F"];  _baseSide = _nearestFlag getVariable ["BASE_SIDE", civilian]; _baseUsers = _nearestFlag getVariable ["BASE_USERS", []]; !(isNull _nearestFlag) && (_nearestFlag distance player < 5) && _baseSide == playerSide && ((getPlayerUID player) in _baseUsers)']];
