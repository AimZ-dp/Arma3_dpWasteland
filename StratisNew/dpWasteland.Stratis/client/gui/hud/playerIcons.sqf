...
addMissionEventHandler ["Draw3D", {
	private ["_playerIcon","_playerPos","_playerName","_playerPosATL","_playerDistance","_iconColour","_iconSize","_target","_uid","_map","_timeOut","_objectDistance","_objectPos","_timeLeft","_lastTimeOut","_strength"];
	...
	if (!visibleMap) then 
	{
		...
		{  
			...
			{
				// 1000 - 1500 meters
				_strength = (-0.0005 * (_playerPos distance player)) + 1;
			};
			...
			{
				case "WEST": {_playerIcon = "\A3\ui_f\data\map\Markers\NATO\b_unknown.paa";_iconColour = [0,0,1,_strength];};
				case "EAST": {_playerIcon = "\A3\ui_f\data\map\Markers\NATO\o_unknown.paa";_iconColour = [1,0,0,_strength];};
				case "GUER": {_playerIcon = "\A3\ui_f\data\map\Markers\NATO\n_unknown.paa";_iconColour = [0,1,0,_strength];};
				default {_playerIcon = "\A3\ui_f\data\map\Markers\Military\unknown_ca.paa";_iconColour = [1,1,1,_strength];};
			};
		...
		{
			{
				...
				// 1500 meters
				_strength = (-0.0015 * (_playerPos distance player)) + 1;
				if (_strength > 1) then {_strength = 1;};
				if (_strength < 0) then {_strength = 0;};
			
				drawIcon3D ["", [1,1,1,_strength], _playerPos, 0, 0, 0, format ["%1 (%2m)", name _x, round (_x distance player)], 0, 0.03, "PuristaLight"];			
			} forEach nameArray;  
			
			{
				...
				// 400 meters
				_strength = (-0.0015 * (_objectPos distance player)) + 1;
				if (_strength > 1) then {_strength = 1;};
				if (_strength < 0) then {_strength = 0;};
				...
				drawIcon3D ["", [1,1,1,_strength], _objectPos, 0, 0, 0, format["respawn in %1s", floor(_timeLeft)], 0, 0.03, "PuristaLight"];		
		...
		{
			{
				...
				drawIcon3D ["", [1,1,1,_strength], _playerPos, 0, 0, 0, name _x, 0, 0.03, "PuristaLight"];			
			} forEach nameArray;
		};
	};
}];
