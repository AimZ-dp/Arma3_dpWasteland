...
// Create a Gun Store in each Zone
{
	if (!(_x in pvar_restrictedZones)) then
	{
		...
		{
			// Check the array 
			_storeGuyPos = pvar_gunStoreLocations select _locIndex;
			...
			if (count _storeGuyPos < 2) then
			{
				...
				["	Weapon Store (zone %1): Generated Pos=%2", _x, _randomPos] call BIS_fnc_logFormat;
			...
			_storeGuy = _gunStoreGroup createunit ["C_man_polo_4_F", _storeGuyPos, [], 1, "NONE"];
			...
			pvar_gunStoreLocations set [_locIndex, _storeGuyPos];
			
			// Control the guy
			[_storeGuy, _storeGuyPos, "gun", _locIndex, _storeHouse] spawn controlStoreGuy;
