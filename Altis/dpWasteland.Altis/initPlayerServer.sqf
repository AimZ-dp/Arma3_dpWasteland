...
// Description:  	Executed only on server when a player joins mission (includes both mission start and JIP). 
//
...
"*** initPlayerServer Started ***" call BIS_fnc_log;

if (!isDedicated) exitWith {["Warning: Shouldn't run initPlayerServer.sqf on client!"] call BIS_fnc_error; false};

private ["_player", "_didJIP"];
_player = [_this, 0, objNull, [objNull]] call BIS_fnc_param;
_didJIP = [_this, 1, false] call BIS_fnc_param;   

["Playing %1 just join the server (%2)",name _player, _didJIP] call BIS_fnc_logFormat;

"*** initPlayerServer Finished ***" call BIS_fnc_log;
