...
_currSpawnBeaconFaction = ((nearestobjects [getpos player, ["Land_SatellitePhone_F"],  5] select 0) getVariable "faction");

if(isNil{_currSpawnBeaconFaction}) exitWith {};

_currSpawnBeacon = (nearestobjects [getpos player, ["Land_SatellitePhone_F"],  5] select 0);
    ...
    case 0:{ // Steal
		...
		for "_iteration" from 1 to _lockDuration do {
			...
			if (_iteration >= _totalDuration) exitWith { // Sleep a little extra to show that steal has completed.
                ...
                _currBeaconTemp = (nearestObjects [getpos player, ["Land_SatellitePhone_F"],  5]);
		        ...
		        if(count _currBeaconTemp == 0) then { // Check if the beacon has been removed since curr player started interacting with it.
                    ...
                    mutexScriptInProgress = false;
	               	...
	               	deleteVehicle (nearestobjects [getpos player, ["Land_SatellitePhone_F"],  5] select 0);
